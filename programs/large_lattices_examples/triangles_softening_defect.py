...
#redundant n1 to implement triangular grid
n2 = 15
n1 = n2 + n2 // 2
n1_real = 10
...
def is_node_func(coords):
    ...
    """
    return (coords[0] - coords[1] // 2 > -1) and (2 * coords[0] - coords[1] < 2 * (n1_real))
...
def xi0_func(coords):
    ...
    """
    delta = 1.
    x_offset=  (1- n1_real)*delta/2.
    (i, j) = coords
    result = (i * delta - j * delta / 2.+ x_offset, j * delta * math.sqrt(3) / 2)

    #defect
    special_i = 6
    special_j = 4
    special_offset = (delta / 2.5, delta / 3.)

    if (i - j // 2 == special_i) and (j == special_j):
        result = (result[0] + special_offset[0],result[1] + special_offset[1])
    return result
    return result
...
def r_and_r_prime_component_functions(node_coords, component):
    ...
    if node_coords[0] == 0: #for the nodes on the vertical axis of symmetry
...
solution = process.solve_e_in_V_linear_softening(e0, t0, dt, nsteps, a0, p0)
...
viewport = ((-6, 6), (-1, 13))
filename = "triangles_softening_defect"
